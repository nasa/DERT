...
public class BasicCamera
	 ...
	 * @param that
	 */
	public BasicCamera(Camera that) {
		super(that);
		_fovY = that.getFovY();
		if (that instanceof BasicCamera) {
			fovX = ((BasicCamera)that).getFovX();
			aspect = ((BasicCamera)that).getAspect();
		}
	}
...
//		}
	 ...
	 * @return
	 */
	public int getMagIndex(double m) {
		for (int i = 0; i < magFactor.length; ++i) {
			if (m < magFactor[i]) {
				return(i);
			}
